...
import { QueryQuestionCommentCategory, QueryQuestionComment, QueryQuestionCommentsApi } from "../../generated/client";
  ...
  */
  ...
  private async onQueryQuestionCommentNotification(message: QueryQuestionCommentNotification) {
    ...
    if (message.type == "CREATED" && parentCommentId == this.props.parentId) {
      const comment = await this.getQueryQuestionCommentsService(this.props.accessToken).findQueryQuestionComment({
      ...
      for (let i = 0; i < this.state.comments.length; i++) {
        ...
        if (message.commentId == comment.id) {
          if (message.type == "UPDATED") {
            comments.push(await this.getQueryQuestionCommentsService(this.props.accessToken).findQueryQuestionComment({
              panelId: this.props.panelId,
              commentId: message.commentId
            }));
          }
  ...
  private async loadChildComments() {
    if (!this.state.comments && this.props.accessToken) {
      ...
      const comments = await (this.getQueryQuestionCommentsService(this.props.accessToken)).listQueryQuestionComments({
        panelId: this.props.panelId,
   ...
   * @returns query question comments API
   */
  private getQueryQuestionCommentsService(accessToken: string): QueryQuestionCommentsApi {
...
function mapStateToProps(state: StoreState) {
  return {
    accessToken: state.accessToken ? state.accessToken.token : undefined,
    loggedUserId: state.accessToken ? state.accessToken.userId : undefined,
    locale: state.locale
  };
}
