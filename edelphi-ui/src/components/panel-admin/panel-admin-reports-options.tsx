...
import { QueryPage, PanelExpertiseGroup, PanelInterestClass, PanelExpertiseClass, QueryQuestionCommentCategory, QueryPagesApi, QueryQuestionCommentCategoriesApi, PanelExpertiseApi } from "../../generated/client";
...
class PanelAdminReportsOptions extends React.Component<Props, State> {
  ...
  public async componentDidMount() {
    ...
    const queryPages = await queryPagesService.listQueryPages({
      includeHidden: true,
      panelId: this.props.panelId,
      queryId: this.props.queryId
    });
    
    const panelExpertiseGroups = await this.getPanelExpertiseService().listExpertiseGroups({
      panelId: this.props.panelId
    });

    const panelInterestClasses = await this.getPanelExpertiseService().listInterestClasses({
      panelId: this.props.panelId
    });

    const panelExpertiseClasses = await this.getPanelExpertiseService().listExpertiseClasses({
      panelId: this.props.panelId
    });
  ...
  private loadCommentCategories = async () => {
    try {
      ...
      return await queryQuestionCommentCategoriesService.listQueryQuestionCommentCategories({
        pageId: 0,
        panelId: this.props.panelId,
        queryId: this.props.queryId
      });
   ...
   * @returns query comment categories API
   */
  private getQueryQuestionCommentCategoriesService(): QueryQuestionCommentCategoriesApi {
   ...
   * @returns panel expertise service
   */
  private getPanelExpertiseService(): PanelExpertiseApi {
...
function mapStateToProps(state: StoreState) {
  return {
    accessToken: state.accessToken ? state.accessToken.token : "",
    logggedUserId: state.accessToken ? state.accessToken.userId : "",
    locale: state.locale
  };
}
