...
import { QueryPage, Panel, User, QueryQuestionAnswer, QueryQuestionComment, QueryQuestionCommentCategory, Query, QueryPageLive2d, QueryQuestionAnswerLive2d, QueryQuestionCommentsApi, QueriesApi, UsersApi, PanelsApi, QueryPagesApi, QueryQuestionAnswersApi, QueryQuestionCommentCategoriesApi } from "../generated/client";
...
interface CommentAndAnswer {
  answer: QueryQuestionAnswerLive2d;
...
class LiveView extends React.Component<Props, State> {
  ...
  public async componentDidMount() {
    ...
    const queries = await this.getQueriesService().listQueries({
      panelId: panelId
    });

    const loggedUser = await this.getUsersService().findUser({
  ...
  */
  ...
  private renderCommentsView = () => {
    ...
    const xLabel = page && page.axisX ? page.axisX.label : "";
    const yLabel = page && page.axisY ? page.axisY.label : "";
  ...
  private async loadData() {
    ...
    if (this.state.queryId) {
      ...
      await this.setStateAsync({
        pages: pages.filter((page: QueryPage) => {
    ...
    if (this.state.pageId && this.state.queryId && this.state.panel && this.state.panel.id) {
      ...
      if (!page || !page.axisX || !page.axisY || !page.axisX.options || !page.axisY.options) {
        throw new Error("Could not lookup page axes");
      }

      await this.setStateAsync({
        categories: await this.getQueryQuestionCommentCategoriesService(this.props.accessToken.token).listQueryQuestionCommentCategories({ 
          pageId: this.state.pageId, panelId: 
          this.state.panel.id }
        ),
        pageMaxX: page.axisX.options.length,
        pageMaxY: page.axisY.options.length
      });
    } else {
      await this.setStateAsync({
        categories: []
    ...
    if (this.state.pageId && this.state.queryId && this.state.panel && this.state.panel.id && this.state.categoryId !== undefined) {
      const comments = await this.getQueryQuestionCommentsService().listQueryQuestionComments({
      ...
      const answers = await this.getQueryQuestionAnswersService().listQueryQuestionAnswersLive2d({
        panelId: this.state.panel.id,
        queryId: this.state.queryId,
        pageId: this.state.pageId 
      });
   ...
   * @returns query question comments API
   */
  private getQueryQuestionCommentsService(): QueryQuestionCommentsApi {
   ...
   * @returns queries API
   */
  private getQueriesService(): QueriesApi {
   ...
   * @returns users API
   */
  private getUsersService(): UsersApi {
   ...
   * @returns query question answers API
   */
  private getQueryQuestionAnswersService(): QueryQuestionAnswersApi {
  ...
  private formatDate(date?: Date | string): string {
    if (date) {
      return moment.default(date).locale(getLanguage()).format("LLL");
    }
   ...
   * @param answer answer
   */
  private updateAnswer = async (answer: QueryQuestionAnswerLive2d) => {
  ...
  private async onQueryQuestionCommentNotification(notification: QueryQuestionCommentNotification) {
    ...
    const comment = await this.getQueryQuestionCommentsService().findQueryQuestionComment({
  ...
  private async onQueryQuestionAnswerNotification(notification: QueryQuestionAnswerNotification) {
    ...
    const answer = await this.getQueryQuestionAnswersService().findQueryQuestionAnswerLive2d({
