...
RUN curl -o /tmp/kubeping-module.zip -L https://static.metatavu.io/wildfly/wildfly-${WILDFLY_VERSION}-kubeping.zip

RUN unzip -o /tmp/mysql-module.zip -d /opt/jboss/wildfly/
RUN unzip -o /tmp/keycloak-module.zip -d /opt/jboss/wildfly/
RUN unzip -o /tmp/kubeping-module.zip -d /opt/jboss/wildfly/
