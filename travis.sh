...
if [ "$TRAVIS_PULL_REQUEST" != "false" ] && [ -n "${GITHUB_TOKEN}" ] && [ -n "${SONAR_TOKEN}" ]; then
  ...
  mvn clean verify -Pui -DskipGoogleTests=true -Dit.browser=chrome-headless -DrepoToken=$COVERALLS_TOKEN -DskipCoverage=false -Dwebdriver.chrome.driver=../chromedriver
  TEST_STATUS=$?
