package fi.metatavu.edelphi.tools;
...
public class DataImporter {
  ...
  public void doImport() throws Exception {
    ...
    if (queryDataFile != null) {
      try (
        InputStreamReader streamReader = new InputStreamReader(queryDataFile.getInputStream());
        CSVReader queryDataReader = new CSVReader(streamReader, delimiter)) {
        
        // Read CSV header
        
        Iterator<String[]> csvIterator = queryDataReader.iterator();
        
        Map<String,Integer> csvParameterIndices = new HashMap<>();
        if (csvIterator.hasNext()) {
          String[] columns = csvIterator.next();
          for (int i = 0; i < columns.length; i++) {
            csvParameterIndices.put(columns[i], i);
          }
        }
        ...
        while (csvIterator.hasNext()) {
          ...
          String creationDateStr = rowData[csvParameterIndices.get(createdParameter)];
          try {
          ...
          String modifiedDateStr = rowData[csvParameterIndices.get(modifiedParameter)];
          try {
          ...
          for (int i = minPage; i <= maxPage; i++) {
            ...
            for (String fieldParameter : fieldParameters) {
              ...
              String oldValue = rowData[csvParameterIndices.get(fieldParameter)];
            ...
            for (String commentParam : commentParameters) {
              String comment = rowData[csvParameterIndices.get(commentParam)];
